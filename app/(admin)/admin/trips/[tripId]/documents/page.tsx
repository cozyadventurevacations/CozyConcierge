...
function parseRouteAirports(value: string | null) {
...
function normalizeFlightDirection(value: string | null, fallback: "outbound" | "return") {
...
function combineFlightDateTime(
...
function getFlightSegment(
  ...
  const [routeDeparture, routeDestination] = parseRouteAirports(route);
  const direction = normalizeFlightDirection(
  ...
  const segment = {
    ...
    departure_datetime: combineFlightDateTime(
    ...
    arrival_datetime: combineFlightDateTime(
...
function getFlightSegments(payload: Record<string, unknown>) {
  const rawSegments =
  ...
  const segments = rawSegments
    .map((segment, index) =>
      getFlightSegment(segment, index, index === 0 ? "outbound" : "return"),
    )
    .filter(Boolean) as Array<NonNullable<ReturnType<typeof getFlightSegment>>>;

  if (segments.length > 0) return segments;

  const [routeDeparture, routeDestination] = parseRouteAirports(
    getExtractedString(payload, ["location_or_route", "route"]),
  );
  const outbound = getFlightSegment(
    {
      direction: "outbound",
  ...
  const returnSegment = getFlightSegment(
    {
      direction: "return",
  ...
  return [outbound, returnSegment].filter(Boolean) as Array<
    NonNullable<ReturnType<typeof getFlightSegment>>
  >;
}

async function upsertFlightSegment(
  supabase: any,
  componentId: string,
  airlineLocator: string | null,
  segment: NonNullable<ReturnType<typeof getFlightSegment>>,
) {
  const segmentPayload = compactPayload({
...
async function applyExtractedBookingDetailsToComponent(
  ...
  if (componentType === "hotel") {
    ...
    const flightSegments = getFlightSegments(payload);
    ...
    for (const segment of flightSegments) {
      await upsertFlightSegment(supabase, document.component_id, confirmationNumber, segment);
    }
