...
export async function POST(request: Request) {
  ...
  const response = await fetch(
    `https://places.googleapis.com/v1/places/${encodeURIComponent(placeId)}`,
    {
      ...
      headers: {
        ...
        "X-Goog-FieldMask":
          "id,displayName,formattedAddress,addressComponents,nationalPhoneNumber,internationalPhoneNumber,websiteUri,googleMapsUri",
      },
    },
  );
  ...
  const country = getComponent(data.addressComponents, "country");
  ...
  return NextResponse.json({
    hotel: {
      ...
      hotelName: data.displayName?.text ?? "",
      addressLine1,
      addressLine2: "",
      city,
      state,
